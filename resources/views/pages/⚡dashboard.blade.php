...
new #[Layout('layouts.app')] #[Title('Dashboard')] class extends Component {
    ...
    public function stats(): array
    {
        ...
        if ($this->isAdmin || $this->isKepalaSekolah) {
            ...
            $studentCount = Student::whereIn('classroom_id', $teacherClassrooms)->where('status', 'aktif')->count();
    ...
    public function studentStatusChart(): array
    {
        ...
        return [
            'labels' => array_map(fn($s) => Student::STATUSES[$s] ?? $s, array_keys($statuses)),
    ...
    public function employeeTypeChart(): array
    {
        ...
        return [
            'labels' => array_map(fn($t) => Employee::TYPES[$t] ?? $t, array_keys($types)),
    ...
    private function getTeacherClassrooms(): array
    {
        ...
        $homeroomClasses = Classroom::where('homeroom_teacher_id', $this->user->id)
            ->where('is_active', true)
            ->pluck('id')
            ->toArray();

        // In future, also get classrooms from schedules
